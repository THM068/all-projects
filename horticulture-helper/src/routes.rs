...
pub async fn chat(State(state): State<AppState>) -> impl IntoResponse {
    let ai_agent = state.agent;
    let result = ai_agent.prompt("what year was Isreael created").await;
    match result {
        Ok(r) => { println!("The result is {}", r);}
        Err(_) => { println!("Error")}
    }
    templates::QuestionNewTemplate{}
}

pub async fn chat_post(State(state): State<AppState>, Form(form): Form<TodoNew>) -> impl IntoResponse {
    let ai_agent = state.agent;
    let result = ai_agent.prompt(&form.description).await;
    match result {
        Ok(r) => { println!("The result is {}", r);}
